...
function calcOverturning_rho_Aocean (expdir,expname,prodir)
...
%           theta_f(i,j,:) = w_p(i,j,:).*theta_v(i,j,squeeze(k_p(i,j,:))) + w_n(i,j,:).*theta_v(i,j,squeeze(k_n(i,j,:)));
        end
      end
    end
...
%   save(fullfile(prodir,[expname,'_MOC_rho_Aocean.mat']), ...
%     'xx','yy','zz','zz_f','hFacS_f','delRf','ptlevs','pt_xtavg','pt_f_xtavg', ... 
%     'vvel_f','pt_tavg','pt_f', ...  
